...
int main()
{
    ...
    /* 改变时间差大小，波浪效果变化 */
    Voyage ocean(wind_direction, wind_velocity, 128, 0.02f, 1);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                ...
                omodel = glm::translate(omodel, glm::vec3(47.50f * (i), 0.0f, 47.50f * (j)));
...
void processInput(GLFWwindow* window)
{
    ...
    /* Blinn-Phong */
    /*if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && !blinnKeyPressed)
    {
        blinn = !blinn;
